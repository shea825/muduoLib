...
template <typename T> void LogStream::formatInteger(T v) {
  if (buffer_.avail() >= kMaxNumericSize) {
    size_t len = convert(buffer_.current(), v); //数字转换为字符串存放
    buffer_.add(len);
  }
}
...
LogStream &LogStream::operator<<(const void *p) {
  ...
  /**
   * @note uintptr_t 对于 32位平台 是 unsigned int
   *                 对于 64位平台 是 unsigned long int
   */
  if (buffer_.avail() >= kMaxNumericSize) {
